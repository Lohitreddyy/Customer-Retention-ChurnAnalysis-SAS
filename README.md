# Customer-Retention-ChurnAnalysis-SAS
...
libname la '\\apporto.com\dfs\IIT\Users\lohitarrabothu\Downloads';
run;
proc print data=la.calendar(obs=100);
run;
proc print data=la.flight_activity(obs=100);
run;
proc print data=la.loyalty_history(obs=100);
run;
proc contents data=la.calendar;
run;
proc contents data=la.flight_activity;
run;
proc contents data=la.loyalty_history;
run;
/* Summary statistics for Customer Flight Activity dataset */
proc means data=la.flight_activity;
    var Total_Flights Distance Points_Accumulated Points_Redeemed;
run;
/* Summary statistics for Customer Loyalty History dataset */
proc means data=la.loyalty_history;
    var Lalary CLV;
run;
/* Frequency counts for demographic and categorical variables in Loyalty History dataset */
proc freq data=la.loyalty_history;
    tables Gender Education Marital_Status Enrollment_Type Loyalty_Card;
run;

/* Check for missing values in Flight Activity dataset */
proc means data=la.flight_activity nmiss;
    var Total_Flights Distance Points_Accumulated Points_Redeemed;
run;
/* Check for missing values in Loyalty History dataset */
proc means data=la.loyalty_history nmiss;
    var Lalary CLV;
run;
/*The Lalary field has a significant number of missing values (4,238 out of 16,737).*/
/*Approach*/
/* Step 1: Analyze the pattern of missing values for Lalary and key variables */
proc means data=la.loyalty_history n nmiss mean std min max;
    var Lalary CLV;
    title 'Analysis of Lalary and CLV';
run;
/* Step 2: Create a flag for missing Lalary values */
/*data loyalty_temp;
    set la.loyalty_history;
    Lalary_Missing = missing(Lalary);
run;*/
/* Step 3: Analyze the relationship between missing Lalary and categorical variables */
/*proc freq data=loyalty_temp;
    tables (Education Gender Marital_Status Enrollment_Type Loyalty_Card) * Lalary_Missing / chisq;
    title 'Missing Lalary Pattern Analysis';
run;
...
/* Step 1: Calculate mean lalaries for College education based on other variables */
proc sql;
    ...
    select 
        ...
        mean(Lalary) as mean_lalary,
        std(Lalary) as std_lalary
    from la.loyalty_history
    where Education ne 'College' /* Use non-College education levels as reference */
    group by 
        Marital_Status,
        Loyalty_Card,
        calculated CLV_Group
    having mean_lalary is not null;
quit;
...
proc sql;
    ...
    select 
        a.*,
        case 
            when missing(a.Lalary) then b.mean_lalary * 
                /* Add some random variation based on std */
                (1 + (ranuni(123) - 0.5) * 0.1)
            else a.Lalary 
        end as Imputed_Lalary
    from loyalty_temp a
    ...
    where not missing(calculated Imputed_Lalary);
quit;

/* Step 4: Create final cleaned dataset */
data la.loyalty_history_clean;
    set loyalty_imputed;
    Lalary = Imputed_Lalary;
    Lalary_Imputed_Flag = Lalary_Missing;
    drop Imputed_Lalary CLV_Group Lalary_Missing;
run;

/* Step 5: Validate imputation */
proc means data=la.loyalty_history_clean n mean std min max;
    class Education;
    var Lalary;
    title 'Validation of Lalary Distribution by Education';
run;

/* Visual validation */
proc sgplot data=la.loyalty_history_clean;
    vbox Lalary / category=Education;
    title 'Lalary Distribution by Education (After Imputation)';
run;

/* Check correlation with CLV */
proc corr data=la.loyalty_history_clean;
    var Lalary CLV;
    title 'Correlation between Lalary and CLV after Imputation';
run;
/* Check for missing values in Loyalty History dataset */
proc means data=la.loyalty_history_clean nmiss;
    var Lalary CLV;
run;
/* Step 1: Create churn indicator and enrollment duration in loyalty_history */
data loyalty_base;
    set la.loyalty_history_clean;
